...
Sanitized configs use `${PI_API_BASE_URL}`, `${PI_PROVIDER_API_KEY}`, `${OPENCODE_API_BASE_URL}`, and `${OPENCODE_API_KEY}`. Set these locally before using installed configs. Pi `settings.json` is intentionally copied verbatim by explicit user approval; review it manually before every commit.
